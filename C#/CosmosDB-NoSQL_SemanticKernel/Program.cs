...
namespace CosmosRecipeGuide
{
    internal class Program
    {
       ...
       private static void PerformSearch(IConfiguration config)
        ...
        {
            string MemoryDBName = "ContosoRecipes_SK";

            string openAIEndpoint = config["OpenAIEndpoint"];
            string openAIKey = config["OpenAIKey"]; ;
            string embedingModel = config["OpenAIEmbeddingDeployment"];
            string completionsModel = config["OpenAIcompletionsDeployment"];
 
            return new SemanticKernelService(openAIEndpoint, openAIKey, embedingModel, completionsModel, cosmosService.client, MemoryDBName);
        }
